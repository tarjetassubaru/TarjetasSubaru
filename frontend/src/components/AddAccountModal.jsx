...
import { createAccount } from "../api/banks";
...
export default function AddAccountModal({ bankId, onClose, onCreated }) {
  ...
  const [isUfIndexed, setIsUfIndexed] = useState(false);
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
      <div className="bg-[#0f1117] border border-gray-800 rounded-xl w-full max-w-sm mx-4 p-6 shadow-2xl">
        <h3 className="text-lg font-semibold text-gray-200 mb-4">Nueva Cuenta</h3>
        <form onSubmit={handleSubmit}>
          ...
          <label className="block mb-1 text-sm text-gray-400">Saldo inicial</label>
          <input
            type="number"
            value={balance}
          ...
          <input
            ...
            min="0"
            max="100"
            className="w-full px-3 py-2.5 bg-[#1a1d2e] border border-gray-700 rounded-lg text-gray-200 text-sm placeholder-gray-500 focus:outline-none focus:border-gray-500 transition-colors mb-1"
          />
          <p className="text-[10px] text-gray-600 mb-3">Deja en 0 si no aplica. Ej: 5 = 5% anual compuesto diario</p>

          {accountType !== "ahorro_premium" && (
            <>
          ...
          {(accountType === "ahorro_premium" || isUfIndexed) && (
            <div className="p-3 bg-blue-500/10 border border-blue-500/20 rounded-lg mb-3">
              ...
              />
              <p className="text-[9px] text-gray-500">Los fondos deben mantenerse 90+ dias para generar intereses</p>
            </div>
          )}
